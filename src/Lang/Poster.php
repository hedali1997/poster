...
class Poster extends Base implements MyPoster
{
     ...
     * @return   [type]                                [description]
     */
    public function buildText($content, $dst_x = 0, $dst_y = 0, $font = 16, $rgba = [], $max_w = 0, $font_family = '', $text_algin='left', $box_w = 100, $weight = 1, $space = 0, $rotate = 0)
    {
    ...
    public function buildTextMany($arr = [])
    {
        foreach ($arr as $key => $value) {
            ...
            $value['text_algin'] = $value['text_algin'] ?? 'left';
            $value['box_w'] = $value['box_w'] ?? 100;
            $value['weight'] = $value['weight'] ?? 1;
            $value['space'] = $value['space'] ?? 0;
            $value['rotate'] = $value['rotate'] ?? 0;

            $this->CopyText($value['content'], $value['dst_x'], $value['dst_y'], $value['font'], $value['rgba'],
                $value['max_w'], $value['font_family'], $value['text_algin'], $value['box_w'], $value['weight'], $value['space'], $value['rotate']);
        }
        return $this;
    }
